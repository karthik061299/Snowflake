...
## *Version*: 2 
## *Changes*: Added connections between tables in the Conceptual Data Model Diagram block format
## *Reason*: User requested to add missing connections between tables in the block diagram to show relationships clearly
## *Updated on*: 
_____________________________________________
...
```
                    ...
                    └─────────────────┘
                            ...
                            ▼
┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐
│  Si_Customers   │    │   Si_Products   │    │  Si_Warehouses  │
├─────────────────┤    ├─────────────────┤    ├─────────────────┤
...
└─────────────────┘    └─────────────────┘    └─────────────────┘
        ...
        ▼                       ▼                       ▼
┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐
│   Si_Orders     │    │  Si_Inventory   │    │ Si_Stock_Levels │
├─────────────────┤    ├─────────────────┤    ├─────────────────┤
│ Order_Date      │    │Quantity_Available│    │Reorder_Threshold│
│ load_timestamp  │    │ load_timestamp  │    │ load_timestamp  │
│ update_timestamp│    │ update_timestamp│    │ update_timestamp│
│ source_system   │    │ source_system   │    │ source_system   │
│data_quality_score│   │data_quality_score│   │data_quality_score│
...
└─────────────────┘    └─────────────────┘    └─────────────────┘
        ...
        ▼                                              ▼
┌─────────────────┐                            ┌─────────────────┐
│ Si_Order_Details│◄──────────────────────────►│   Si_Products   │
├─────────────────┤        (Product_Name)      │  (Referenced)   │
│ Quantity_Ordered│                            └─────────────────┘
│ load_timestamp  │
│ update_timestamp│
│ source_system   │
...
└─────────────────┘
        ...
        DATA QUALITY & AUDIT MANAGEMENT LAYER
        ═══════════════════════════════════════

┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐
│Si_Data_Quality_ │    │Si_Data_Validation│   │Si_Pipeline_Audit│
│     Errors      │    │     Rules       │    │      Log        │
├─────────────────┤    ├─────────────────┤    ├─────────────────┤
│ error_id        │    │ rule_id         │    │ audit_id        │
│ source_table    │◄───┤ rule_name       │    │ pipeline_name   │
│ source_record_id│    │ rule_description│    │ pipeline_run_id │
│ error_type      │    │ target_table    │────►│execution_start_ │
│ error_description│   │ target_field    │    │     time        │
│ error_field     │    │ rule_type       │    │execution_end_   │
│ error_value     │    │ rule_expression │    │     time        │
│ error_severity  │    │ is_active       │    │execution_duration│
│ error_timestamp │    │created_timestamp│    │execution_status │
│resolution_status│    │updated_timestamp│    │ source_table    │
│resolution_notes │    └─────────────────┘    │ target_table    │
│ created_by      │                           │records_processed│
└─────────────────┘                           │records_successful│
        │                                     │ records_failed  │
        │ (source_table)                      │ records_skipped │
        ▼                                     │ data_volume_mb  │
┌─────────────────┐                           │ error_message   │
│ ALL Si_ TABLES  │                           │ executed_by     │
...
**Detailed Relationship Connections:**

### **Core Business Entity Relationships:**
1. **Si_Suppliers → Si_Products** (via Product_Name field)
   - Suppliers provide products to the inventory system

2. **Si_Products → Si_Inventory** (via Product_Name field)
   - Products have inventory quantities tracked in warehouses

3. **Si_Products → Si_Order_Details** (via Product_Name field)
   - Products are ordered through order details

4. **Si_Products → Si_Stock_Levels** (via Product_Name field)
   - Products have reorder thresholds defined for stock management

5. **Si_Warehouses → Si_Inventory** (via Location field)
   - Warehouses store inventory at specific locations

6. **Si_Warehouses → Si_Stock_Levels** (via Location field)
   - Stock levels are maintained per warehouse location

7. **Si_Customers → Si_Orders** (via Customer_Name field)
   - Customers place orders in the system

8. **Si_Orders → Si_Order_Details** (via Order_Date field)
   - Orders contain multiple order detail line items

9. **Si_Orders → Si_Shipments** (via Order_Date field)
   - Orders are fulfilled through shipments

10. **Si_Orders → Si_Returns** (via Order_Date field)
    - Orders can have associated returns

### **Data Quality & Audit Relationships:**
11. **Si_Data_Quality_Errors → All Si_ Tables** (via source_table field)
    - Error tracking references any Silver layer table

12. **Si_Data_Validation_Rules → All Si_ Tables** (via target_table field)
    - Validation rules apply to specific Silver layer tables

13. **Si_Pipeline_Audit_Log → All Si_ Tables** (via source_table and target_table fields)
    - Audit logs track pipeline operations on Silver layer tables

14. **Si_Process_Monitoring → Si_Pipeline_Audit_Log** (via process_name field)
    - Process monitoring tracks pipeline execution performance
...
- **apiCost:** 0.200000 (Cost consumed by the API for this call in USD)